...
- **Error boundaries** — `{% try %}` blocks catch render errors and fall back to safe defaults without aborting the whole page.
...
- **Error boundaries** — `{% try %}...{% fallback %}...{% end %}` catches exceptions
